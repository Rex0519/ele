...
> **一句话概括**：这是一个"用仿真数据 + AI 对话 + 自动告警"的电力监控 Demo 系统，让管理层不用懂 SQL 也能查用电数据。

## 项目定位

- **核心用途**：基于真实历史数据，自动生成符合实际用电规律的模拟电力数据，并为管理层提供 AI 自然语言查询和智能告警能力。
- **目标用户**：不熟悉 SQL 的管理层（通过 FlowiseAI 对话查询），以及需要接入电力数据的 AI Agent（通过 MCP / REST API / CSV）。
- **典型场景**：
  - 管理层用自然语言问"今天西北区空调用电量同比昨天如何？"
  - AI Agent 通过 MCP 工具查询实时数据、告警、区域汇总
  - 异常用电自动触发告警，HIGH/CRITICAL 级别通过飞书/短信通知
  - OpenClaw Agent 基于每日导出的 CSV 做 pandas 离线分析
